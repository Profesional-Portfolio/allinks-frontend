...
export const LinksPage: React.FC = () => {
  const { links, isLoading, error, getLinks, createLink, deleteLink, changeVisibility } = useLinks();
  ...
  const handleCreateLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ title: "", url: "", description: "", visibility: "public" });
    } catch (err) {
    ...
    <div className="links-page">
      <header className="links-header">
        <div>
          ...
          <p className="page-subtitle">Manage your personal collection of links</p>
        </div>
      ...
      <div className="links-grid">
        {isLoading && links.length === 0 ? (
          ...
          links.map((link) => (
            <div key={link.id} className={`link-card ${link.visibility}`}>
              <div className="link-card-header">
                <h3 className="link-title">{link.title}</h3>
                <div className="link-actions">
                  <button onClick={() => changeVisibility(link.id)} className="btn-icon" title="Toggle Visibility">
                  ...
                  <button onClick={() => deleteLink(link.id)} className="btn-icon btn-delete" title="Delete">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="link-card-footer">
                <span className={`badge ${link.visibility}`}>{link.visibility}</span>
              </div>
            </div>
          ))
        )}
